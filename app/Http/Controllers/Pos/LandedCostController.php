...
class LandedCostController extends Controller
{
    ...
    {
        $costs = LandedCost::where('company_id', $this->companyId())->with('goodsReceipt')->latest()->paginate(30);
        return view('backend.purchase.landed_costs', compact('costs'));
    }

    public function create()
    {
        $receipts = GoodsReceipt::where('company_id', $this->companyId())->where('status', 'approved')->with(['purchaseOrder.supplier', 'lines.product'])->latest()->get();
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate(['cost_no' => ['nullable', 'string', 'max:100', Rule::unique('landed_costs', 'cost_no')->where('company_id', $companyId)], 'goods_receipt_id' => ['required', 'integer', Rule::exists('goods_receipts', 'id')->where('company_id', $companyId)], 'cost_type' => ['required', 'string', 'max:50'], 'amount' => ['required', 'numeric', 'gt:0'], 'allocation_method' => ['required', 'in:by_value,by_quantity'], 'description' => ['nullable', 'string', 'max:2000']]);
        $receipt = GoodsReceipt::where('company_id', $companyId)->findOrFail($data['goods_receipt_id']);
    ...
    {
        try { app(\App\Services\ApprovalGuard::class)->assertBeforeTransaction(LandedCost::class, $id); $cost = LandedCost::where('company_id', $this->companyId())->with('allocations')->findOrFail($id); if ($cost->status !== 'pending') throw new \RuntimeException('This landed cost has already been processed.'); app(\App\Services\ApprovalGuard::class)->assertDifferent($cost); app(LandedCostService::class)->approve($cost); app(AuditService::class)->record('landed_cost.approved', $cost, ['status' => 'pending'], ['status' => 'approved']); return back()->with(['message' => 'Landed cost allocated to inventory layers.', 'alert-type' => 'success']); }
    ...
    public function reject(Request $request, int $id)
    {
        ...
        try {
            $cost = LandedCost::where('company_id', $this->companyId())->findOrFail($id);
