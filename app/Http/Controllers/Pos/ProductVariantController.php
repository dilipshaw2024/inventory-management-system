...
class ProductVariantController extends Controller
{
    ...
    public function storeVariant(Request $request)
    {
        ...
        $data = $request->validate(['parent_product_id' => ['required', 'integer', $productScope], 'name' => ['required', 'string', 'max:255'], 'sku' => ['nullable', 'string', 'max:100', Rule::unique('products', 'sku')->where(fn ($query) => $query->where('company_id', $companyId)->orWhereNull('company_id'))], 'barcode' => ['nullable', 'string', 'max:100', Rule::unique('products', 'barcode')->where(fn ($query) => $query->where('company_id', $companyId)->orWhereNull('company_id'))], 'purchase_price' => ['nullable', 'numeric', 'min:0'], 'sales_price' => ['nullable', 'numeric', 'min:0'], 'tax_rate' => ['nullable', 'numeric', 'min:0', 'max:100'], 'lifecycle_status' => ['nullable', 'in:draft,active,discontinued,blocked,archived'], 'can_purchase' => ['nullable', 'boolean'], 'can_sell' => ['nullable', 'boolean'], 'attribute_value_id' => ['nullable', 'array'], 'attribute_value_id.*' => ['nullable', Rule::exists('product_attribute_values', 'id')->where(fn ($query) => $query->where('company_id', $companyId)->orWhereNull('company_id'))]]);
        $variant = DB::transaction(function () use ($data): Product {
            ...
            $variant = Product::create(['company_id' => $parent->company_id ?: auth()->user()?->company_id, 'parent_product_id' => $parent->id, 'is_variant' => true, 'name' => $data['name'], 'supplier_id' => $parent->supplier_id, 'unit_id' => $parent->unit_id, 'category_id' => $parent->category_id, 'brand_id' => $parent->brand_id, 'sku' => $data['sku'] ?? null, 'barcode' => $data['barcode'] ?? null, 'purchase_price' => $data['purchase_price'] ?? $parent->purchase_price, 'sales_price' => $data['sales_price'] ?? $parent->sales_price, 'tax_rate' => $data['tax_rate'] ?? $parent->tax_rate, 'tracking_type' => $parent->tracking_type, 'product_type' => $parent->product_type ?: 'stock', 'lifecycle_status' => $data['lifecycle_status'] ?? ($parent->lifecycle_status ?: 'active'), 'can_purchase' => (bool) ($data['can_purchase'] ?? ($parent->can_purchase ?? true)), 'can_sell' => (bool) ($data['can_sell'] ?? ($parent->can_sell ?? true)), 'is_stock_item' => (bool) ($parent->is_stock_item ?? true), 'weight_kg' => $parent->weight_kg, 'length_m' => $parent->length_m, 'width_m' => $parent->width_m, 'height_m' => $parent->height_m, 'quantity' => 0, 'status' => 1, 'created_by' => auth()->id()]);
    ...
    {
        $companyId = auth()->user()?->company_id;
        $variant = Product::where('is_variant', true)->lockForUpdate()->findOrFail($id);
        $data = $request->validate([
            'name' => ['required', 'string', 'max:255'],
            'sku' => ['nullable', 'string', 'max:100', Rule::unique('products', 'sku')->ignore($variant->id)->where(fn ($query) => $query->where('company_id', $companyId)->orWhereNull('company_id'))],
            'barcode' => ['nullable', 'string', 'max:100', Rule::unique('products', 'barcode')->ignore($variant->id)->where(fn ($query) => $query->where('company_id', $companyId)->orWhereNull('company_id'))],
            'purchase_price' => ['nullable', 'numeric', 'min:0'], 'sales_price' => ['nullable', 'numeric', 'min:0'],
            'tax_rate' => ['nullable', 'numeric', 'min:0', 'max:100'], 'lifecycle_status' => ['required', 'in:draft,active,discontinued,blocked,archived'],
            'can_purchase' => ['nullable', 'boolean'], 'can_sell' => ['nullable', 'boolean'],
        ]);
        if (app(ProductLifecycleService::class)->hasStock($variant) && $data['lifecycle_status'] === 'archived') abort(422, 'A variant with stock cannot be archived.');
        $fields = ['name', 'sku', 'barcode', 'purchase_price', 'sales_price', 'tax_rate', 'lifecycle_status', 'can_purchase', 'can_sell'];
        $before = $variant->only($fields);
        $variant->update($data + ['updated_by' => auth()->id()]);
        app(AuditService::class)->record('product_variant.updated', $variant, $before, $variant->fresh()->only($fields));
        return back()->with(['message' => 'Product variant updated.', 'alert-type' => 'success']);
    }
}
