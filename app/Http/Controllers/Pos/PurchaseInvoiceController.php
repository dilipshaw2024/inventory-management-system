...
class PurchaseInvoiceController extends Controller
{
    ...
    {
        $invoices = PurchaseInvoice::where('company_id', $this->companyId())->with('supplier')->latest()->paginate(30);
        return view('backend.purchase.invoice_all', compact('invoices'));
    }

    public function create()
    {
        $orders = PurchaseOrder::where('company_id', $this->companyId())->whereIn('status', ['approved', 'partially_received', 'received'])->with(['supplier', 'lines.product'])->latest()->get();
    ...
    {
        $data = $request->validate(['supplier_id' => ['nullable', 'integer', Rule::exists('suppliers', 'id')->where('company_id', $this->companyId())], 'minimum_percent' => ['nullable', 'numeric', 'min:0', 'max:1000']]);
        $invoices = PurchaseInvoice::where('company_id', $this->companyId())->with(['supplier', 'lines.product', 'lines.purchaseOrderLine'])
    ...
    public function store(Request $request)
    {
        ...
        $supplier = Supplier::where('company_id', $companyId)->findOrFail($order->supplier_id);
    ...
    {
        try { app(\App\Services\ApprovalGuard::class)->assertBeforeTransaction(PurchaseInvoice::class, $id); $invoice = PurchaseInvoice::where('company_id', $this->companyId())->with(['lines.product'])->findOrFail($id); if ($invoice->status !== 'pending') throw new \RuntimeException('This invoice has already been processed.'); app(\App\Services\ApprovalGuard::class)->assertDifferent($invoice); app(PurchaseInvoiceService::class)->approve($invoice); app(AuditService::class)->record('purchase_invoice.approved', $invoice, ['status' => 'pending'], ['status' => 'approved']); return back()->with(['message' => 'Purchase invoice approved and posted to accounts payable.', 'alert-type' => 'success']); }
    ...
    public function reject(Request $request, int $id)
    {
        ...
        try {
            $invoice = PurchaseInvoice::where('company_id', $this->companyId())->findOrFail($id);
