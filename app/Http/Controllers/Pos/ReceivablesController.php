...
class ReceivablesController extends Controller
{
    ...
    {
        $invoices = Invoice::with('customer')->where('status', 1)->whereNotNull('customer_id')->where(function ($query) use ($asOf): void { $query->whereDate('date', '<=', $asOf)->orWhere(function ($fallback) use ($asOf): void { $fallback->whereNull('date')->whereDate('created_at', '<=', $asOf); }); })->latest('date')->get();
        $invoiceIds = $invoices->pluck('id');
        $paid = $invoiceIds->isEmpty() ? collect() : Payment::whereIn('invoice_id', $invoiceIds)->where('is_reversed', false)->whereDate('created_at', '<=', $asOf)->selectRaw('invoice_id, COALESCE(SUM(paid_amount), 0) AS amount')->groupBy('invoice_id')->pluck('amount', 'invoice_id');
        $allocated = $invoiceIds->isEmpty() ? collect() : CustomerPaymentAllocation::whereIn('invoice_id', $invoiceIds)->whereNull('voided_at')->whereDate('allocated_at', '<=', $asOf)->whereHas('payment', fn ($query) => $query->where('is_reversed', false))->selectRaw('invoice_id, COALESCE(SUM(amount), 0) AS amount')->groupBy('invoice_id')->pluck('amount', 'invoice_id');
        ...
        $legacyRows = Payment::with(['customer', 'invoice'])->where('due_amount', '>', 0)->where('is_reversed', false)->whereDate('created_at', '<=', $asOf)->whereNotIn('invoice_id', $legacyInvoiceIds)->latest()->get()->map(function (Payment $payment) use ($asOf): array {
