...
class SupplierPaymentController extends Controller
{
    ...
    public function index()
    {
        ...
        $payments = SupplierPayment::where('company_id', $companyId)->with(['supplier', 'purchaseInvoice', 'allocations.invoice'])->latest()->paginate(30);
        $invoices = PurchaseInvoice::where('company_id', $companyId)->where('status', 'approved')->with(['supplier', 'lines'])->latest()->get();
    ...
    public function store(Request $request)
    {
        ...
        $invoice = !empty($data['purchase_invoice_id']) ? PurchaseInvoice::where('company_id', $companyId)->findOrFail($data['purchase_invoice_id']) : null;
        ...
        $payment = SupplierPayment::create($data + ['company_id' => $companyId, 'currency_code' => $currency, 'exchange_rate' => $rate, 'base_amount' => (float) $data['amount'] * $rate, 'payment_no' => $data['payment_no'] ?: app(NumberingSequenceService::class)->nextOrFallback('supplier_payment', 'PAY-'.now()->format('YmdHis').'-'.random_int(100, 999), auth()->user()?->company_id, auth()->user()?->branch_id), 'supplier_id' => $data['supplier_id'], 'status' => 'pending', 'created_by' => auth()->id()]);
    ...
    public function allocate(Request $request)
    {
        ...
        try {
            $allocation = app(SupplierPaymentService::class)->allocate(SupplierPayment::where('company_id', $companyId)->findOrFail($data['payment_id']), (int) $data['purchase_invoice_id'], (float) $data['amount'], null, isset($data['exchange_rate']) ? (float) $data['exchange_rate'] : null);
        } catch (\RuntimeException $exception) {
    ...
    public function voidAllocation(Request $request, int $id)
    {
        ...
        $allocation = SupplierPaymentAllocation::where('company_id', $this->companyId())->findOrFail($id);
        try { $allocation = app(SupplierPaymentService::class)->voidAllocation($allocation->id, $data['void_reason']); }
    ...
    {
        $invoices = PurchaseInvoice::where('company_id', $this->companyId())->with('supplier')->where('status', 'approved')->where(function ($query) use ($asOf): void { $query->whereDate('invoice_date', '<=', $asOf)->orWhere(function ($fallback) use ($asOf): void { $fallback->whereNull('invoice_date')->whereDate('created_at', '<=', $asOf); }); })->orderBy('due_date')->get();
    ...
    {
        try { app(\App\Services\ApprovalGuard::class)->assertBeforeTransaction(SupplierPayment::class, $id); $payment = SupplierPayment::where('company_id', $this->companyId())->findOrFail($id); if ($payment->status !== 'pending') throw new \RuntimeException('This payment has already been processed.'); app(\App\Services\ApprovalGuard::class)->assertDifferent($payment); app(SupplierPaymentService::class)->approve($payment); app(AuditService::class)->record('supplier_payment.approved', $payment, ['status' => 'pending'], ['status' => 'approved']); return back()->with(['message' => 'Supplier payment approved and journalized.', 'alert-type' => 'success']); }
        catch (\RuntimeException $exception) { return back()->with(['message' => $exception->getMessage(), 'alert-type' => 'error']); }
    }
    ...
    public function reject(Request $request, int $id)
    {
        ...
        $payment = SupplierPayment::where('company_id', $this->companyId())->findOrFail($id);
