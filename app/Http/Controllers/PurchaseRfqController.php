...
class PurchaseRfqController extends Controller
{
    ...
    {
        $rfqs = PurchaseRfq::with(['lines.product', 'suppliers.supplier', 'suppliers.quotations'])->latest()->paginate(30);
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            'rfq_no' => ['nullable', 'string', 'max:100', 'unique:purchase_rfqs,rfq_no'],
        ...
        DB::transaction(function () use ($data, &$rfq): void {
            $rfq = PurchaseRfq::create([
                ...
                'rfq_no' => $data['rfq_no'] ?: app(NumberingSequenceService::class)->nextOrFallback('purchase_rfq', 'RFQ-'.now()->format('YmdHis').'-'.random_int(100, 999), auth()->user()?->company_id, auth()->user()?->branch_id),
    ...
    {
        $rfq = PurchaseRfq::with('lines.product')->findOrFail($id);
    ...
    {
        $rfq = PurchaseRfq::with(['lines.product', 'suppliers.supplier', 'suppliers.quotations'])->findOrFail($id);
    ...
    {
        $data = $request->validate(['rfq_supplier_id' => ['required', 'exists:purchase_rfq_suppliers,id']]);
        $rfq = PurchaseRfq::with('lines')->findOrFail($id);
