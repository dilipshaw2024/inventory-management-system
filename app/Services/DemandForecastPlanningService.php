...
class DemandForecastPlanningService
{
    public function forecastsForCompany(int $companyId, int $historyDays = 90, int $horizonDays = 30, ?int $productId = null, ?int $locationId = null, string $seasonality = 'none'): array
    {
        ...
        $selectedModels = [];
        $forecasts = $products->map(function (Product $product) use ($sales, $historyDays, $horizonDays, $availability, $overrides, $from, $forecastFrom, $locationId, $seasonality, &$selectedModels): array {
            ...
            $forecastService = app(DemandForecastService::class); $selection = $seasonality === 'auto' ? $forecastService->selectModel($dailySeries, now()->parse($from), $horizonDays, $daily) : ['model' => in_array($seasonality, ['weekly', 'exponential'], true) ? $seasonality : 'naive'];
            $model = $selection['model']; $selectedModels[$model] = ($selectedModels[$model] ?? 0) + 1;
            $override = $overrides->get($product->id); $forecast = $override ? (float) $override->forecast_quantity : ($model === 'weekly' ? $forecastService->seasonalForecast($dailySeries, now()->parse($from), now()->parse($forecastFrom), $horizonDays, $daily) : ($model === 'exponential' ? $forecastService->exponentialForecast($dailySeries, $horizonDays, $daily) : $daily * $horizonDays)); $confidence = $forecastService->confidenceBand($dailySeries, $daily, $horizonDays, $forecast); $available = (float) ($availability[$product->id] ?? 0);
            return ['product' => $product, 'location_id' => $locationId, 'historical_qty' => $sold, 'daily_rate' => $daily, 'forecast_qty' => $forecast, 'projected_balance' => $available - $forecast, 'reorder_level' => (float) $product->reorder_level, 'confidence_low' => $confidence['low'], 'confidence_high' => $confidence['high'], 'model' => $model, 'model_errors' => ['naive' => $selection['naive_error'] ?? null, 'weekly' => $selection['weekly_error'] ?? null, 'exponential' => $selection['exponential_error'] ?? null], 'model_metrics' => $selection['metrics'] ?? [], 'override' => $override];
        ...
        return ['forecasts' => $forecasts, 'history_days' => $historyDays, 'horizon_days' => $horizonDays, 'seasonality' => $seasonality, 'selected_models' => $selectedModels, 'from' => $from, 'to' => $to, 'forecast_from' => $forecastFrom, 'forecast_to' => $forecastTo];
    }
}
