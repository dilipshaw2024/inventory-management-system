...
class ForeignCurrencyRevaluationService
{
    public function openBalances(string $asOf, ?int $companyId = null): Collection
    {
        ...
        Invoice::where('status', 1)->whereDate('date', '<=', $asOf)->whereNotNull('currency_code')->when($companyId !== null, fn ($query) => $query->where(fn ($scope) => $scope->where('company_id', $companyId)->orWhereNull('company_id')))->get()->each(function (Invoice $invoice) use ($asOf, $base, $rows): void {
            $this->append($rows, 'Accounts receivable', $invoice->invoice_no, $invoice->currency_code, (float) $invoice->total_amount - (float) Payment::where('invoice_id', $invoice->id)->where('approval_status', 'approved')->where('is_reversed', false)->where(fn ($query) => $query->whereDate('payment_date', '<=', $asOf)->orWhere(fn ($legacy) => $legacy->whereNull('payment_date')->whereDate('created_at', '<=', $asOf)))->sum('paid_amount') - (float) CustomerPaymentAllocation::where('invoice_id', $invoice->id)->whereNull('voided_at')->whereHas('payment', fn ($query) => $query->where('approval_status', 'approved')->where('is_reversed', false))->whereDate('allocated_at', '<=', $asOf)->sum('amount'), (float) ($invoice->exchange_rate ?: 1), $asOf, $base, $invoice->id);
        });
