...
class MrpPlanningService
{
    public function proposalsForCompany(int $companyId): array
    {
        ...
        $proposals = collect($requirements)->map(function (float $required, int $productId) use ($products, $companyId, $openPurchase, $openProduction): ?array {
            $product = $products->get($productId); if (!$product) return null;
            $onHand = app(InventoryAvailabilityService::class)->available($product, true, null, $companyId); $purchaseSupply = (float) ($openPurchase[$productId] ?? 0); $productionSupply = (float) ($openProduction[$productId] ?? 0); $netAvailable = $onHand + $purchaseSupply + $productionSupply; $shortage = max(0, $required - $netAvailable);
            $price = $product->supplier_id ? app(SupplierProductPriceService::class)->bestFor($product->supplier, $product, $shortage, now()->toDateString(), null) : null;
            return ['product' => $product, 'product_id' => $product->id, 'required' => $required, 'on_hand' => $onHand, 'open_purchase_quantity' => $purchaseSupply, 'open_production_quantity' => $productionSupply, 'net_available' => $netAvailable, 'shortage' => $shortage, 'price' => $price, 'action' => $product->supplier_id ? 'Buy' : 'Review'];
