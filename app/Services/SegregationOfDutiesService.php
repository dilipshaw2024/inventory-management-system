...
class SegregationOfDutiesService
{
    public function assertCanAssign(User $user, Role $role): void
    {
        ...
        $conflict = RoleConflict::where('is_active', true)->where('role_id', $role->id)->whereIn('conflicting_role_id', $roleIds)->with('conflictingRole')->first()
            ?? RoleConflict::where('is_active', true)->where('conflicting_role_id', $role->id)->whereIn('role_id', $roleIds)->with('role')->first();
