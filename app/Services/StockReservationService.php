...
class StockReservationService
{
    public function batchAllocations(int $salesOrderLineId, float $quantity, ?int $explicitBatchId = null, ?int $locationId = null, ?int $productId = null): Collection
    {
        ...
        $remaining = $quantity;
        $allocations = collect();
        $reservations = StockReservation::where('sales_order_line_id', $salesOrderLineId)->where('status', 'active')->when($productId !== null, fn ($query) => $query->where('product_id', $productId))->when($locationId !== null, fn ($query) => $query->where(fn ($nested) => $nested->whereNull('location_id')->orWhere('location_id', $locationId)))->orderBy('id')->lockForUpdate()->get();
    ...
    public function reserveSalesOrder(SalesOrder $order): void
    {
        DB::transaction(function () use ($order): void {
            foreach ($order->lines as $line) {
                ...
                if ($reserve > 0.000001) {
                    $this->createBatchReservations($product, $reserve, $order->location_id, ['sales_order_line_id' => $line->id]);
                }
            }
        });
    }

    private function createBatchReservations(Product $product, float $quantity, ?int $locationId, array $attributes = []): void
    {
        $base = $attributes + ['product_id' => $product->id, 'location_id' => $locationId, 'created_by' => auth()->id()];
        ...
        $reserved = StockReservation::where('product_id', $product->id)->where('location_id', $locationId)->where('status', 'active')->whereIn('batch_id', $batches->keys())->get()->groupBy('batch_id')->map(fn ($rows) => (float) $rows->sum(fn ($row) => $row->open_quantity));
        $remaining = $quantity;
    ...
    {
        $remaining = $quantity;
        $reservations = StockReservation::where('sales_order_line_id', $lineId)->where('status', 'active')->when($productId !== null, fn ($query) => $query->where('product_id', $productId))->orderBy('id')->lockForUpdate()->get();
        foreach ($reservations as $reservation) {
    ...
    {
        $reservations = StockReservation::whereIn('sales_order_line_id', $order->lines->pluck('id'))->where('status', 'active')->lockForUpdate()->get();
        foreach ($reservations as $reservation) {
    ...
    {
        StockReservation::where('source_type', $order->getMorphClass())->where('source_id', $order->id)->where('status', 'active')->update(['released_quantity' => DB::raw('quantity'), 'status' => 'released']);
    }

    public function releaseForSourceQuantity(ProductionOrder $order, float $quantity): void
    {
        $remaining = $quantity;
        $reservations = StockReservation::where('source_type', $order->getMorphClass())->where('source_id', $order->id)->where('status', 'active')->orderBy('id')->lockForUpdate()->get();
        foreach ($reservations as $reservation) {
            if ($remaining <= 0.000001) break;
    ...
    public function releaseForSourceRequirements(ProductionOrder $order, array $requirements): void
    {
        foreach ($requirements as $productId => $quantity) {
            ...
            $reservations = StockReservation::where('source_type', $order->getMorphClass())->where('source_id', $order->id)->where('product_id', $productId)->where('status', 'active')->orderBy('id')->lockForUpdate()->get();
            foreach ($reservations as $reservation) {
                if ($remaining <= 0.000001) break;
                $release = min($remaining, (float) $reservation->open_quantity);
                $reservation->released_quantity = (float) $reservation->released_quantity + $release;
