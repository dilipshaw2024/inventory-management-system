...
Current migration head: `2026_09_13_000261`. The older migration-summary paragraphs below retain historical release notes; apply the complete migration sequence in filename order.
...
Run `php artisan erp:retention:process` to archive eligible audit/revision records while honoring active legal holds. The command is non-destructive by default. A source purge requires both `purge_enabled` on the policy and the explicit `--purge` option:

```bash
php artisan erp:retention:process --purge
```
...
Complete database-backed tests, migration rehearsal, backup/restore validation, and production scheduler monitoring before treating the application as a high-trust or high-volume deployment.
