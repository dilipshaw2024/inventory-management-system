...
@section('admin')
 ...
 <div class="page-content">
    ...
    <tr>
        <td> {{ $key+1}} </td>
        <td> {{ data_get($item, 'customer.name', 'N/A') }} </td> 
        <td> #{{ data_get($item, 'invoice.invoice_no', 'N/A') }}   </td> 
        <td> {{ data_get($item, 'invoice.date') ? date('d-m-Y',strtotime(data_get($item, 'invoice.date'))) : 'N/A' }} </td> 
