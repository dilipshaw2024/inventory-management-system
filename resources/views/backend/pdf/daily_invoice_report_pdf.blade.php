      ...
      @php
        ...
        <tr>
           <td class="text-center">{{ $key+1 }}</td>
            <td class="text-center">{{ data_get($item, 'payment.customer.name', 'N/A') }}</td>
            <td class="text-center">#{{ $item->invoice_no }}</td>
            <td class="text-center">{{ date('d-m-Y',strtotime($item->date)) }}</td>
            <td class="text-center">{{ $item->description }}</td>
            <td class="text-center">{{ data_get($item, 'payment.total_amount', 0) }}</td>
            
            
        </tr>
         @php
        $total_sum += (float) data_get($item, 'payment.total_amount', 0);
        @endphp
        @endforeach
