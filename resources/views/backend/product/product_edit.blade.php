...
<div class="col-12">
 ...
 <form method="post" action="{{ route('product.update') }}" id="myForm" >
  ...
  <!-- end row -->
            ...
            <div class="row mb-3"><label class="col-sm-2 col-form-label">Inventory costing</label><div class="col-sm-10 row g-2"><div class="col-md-6"><select name="costing_method" class="form-select">@foreach(['fifo' => 'FIFO', 'weighted_average' => 'Weighted average', 'moving_average' => 'Moving average', 'standard' => 'Standard cost'] as $value => $label)<option value="{{ $value }}" {{ ($product->costing_method ?: 'fifo') === $value ? 'selected' : '' }}>{{ $label }}</option>@endforeach</select></div><div class="col-md-6"><input name="standard_cost" value="{{ $product->standard_cost }}" type="number" step="0.000001" min="0" placeholder="Standard cost" class="form-control"></div></div></div>
