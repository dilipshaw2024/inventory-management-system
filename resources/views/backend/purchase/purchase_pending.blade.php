    ...
    <a href="{{ route('purchase.add') }}" class="btn btn-dark btn-rounded waves-effect waves-light" style="float:right;"><i class="fas fa-plus-circle">  Purchase Pending </i></a> <br>  <br>               
            ...
            <tr>
                ...
                <td> {{ date('d-m-Y',strtotime($item->date))  }} </td> 
                 ...
                 <td> {{ data_get($item, 'product.name', 'N/A') }} </td> 
                ...
                <td> 
@if($item->status == '0')
<form method="POST" action="{{ route('purchase.approve',$item->id) }}" class="d-inline" onsubmit="return confirm('Approve this purchase and increase stock?');"> @csrf <button type="submit" class="btn btn-danger sm" title="Approved"><i class="fas fa-check-circle"></i></button> </form>
@endif
                </td>
